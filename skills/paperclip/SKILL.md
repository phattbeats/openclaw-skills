...
description: >
  ...
  endpoints. Also use when triaging an OCPlatform gateway adapter recovery wake
  (agent status=running with errorReason="OCPlatform gateway run failed") on a
  PHATT TECH agent — see OCPlatform Gateway Adapter section below.
---

## OCPlatform Gateway Adapter

This team's agents (Ledger, House, Van Dam, Monet, Jenkins) run as `adapterType: "ocplatform_gateway"`, NOT local-adapter. The protocol below assumes local-adapter (Claude/Codex spawned by Paperclip directly, with `PAPERCLIP_AGENT_ID`/`PAPERCLIP_RUN_ID`/`PAPERCLIP_API_KEY` auto-injected). We do not get those env vars, so:

- **No `X-Paperclip-Run-Id` audit trail.** Don't fabricate one — if it's empty, just omit the header. Paperclip's liveness monitor sees our runs as "dead" once `waitTimeoutMs=600000` (10 min) hits, regardless. This is a structural limitation of the gateway-adapter model, not something the skill can fix.

- **Long jobs hit the 10-min adapter kill-switch.** Anything over 10 minutes (the configured `waitTimeoutMs` on Ledger's OCPlatform adapter) gets reported as `errorReason="OCPlatform gateway run failed"` and status gets stuck at `running`. Scope tasks smaller, or split them.

- **Recovery pattern (this is the important part).** If you find an agent in `status="running"` with `errorReason="OCPlatform gateway run failed"`:
  1. `PATCH /api/agents/{agentId}` with `{ "status": "idle" }` — clears the dispatcher's precondition check.
  2. Verify `ws://OCPlatform:18789` is reachable from the host (`curl -m 5 http://OCPlatform:18789/` — any 2xx/4xx response means the service is alive).
  3. Reassign the issue to the target agent so Paperclip re-wakes them.
  4. Comment on the issue with what you tried and what the next agent should do, so the next wake has context.

- **Session key for Ledger:** `agent:main:paperclip` (specific to our OCPlatform gateway wiring). Don't assume other agents use the same pattern.

The official skill's full protocol (checkouts, run IDs, scoped wakes, etc.) still applies wherever our adapter doesn't diverge — this section only lists the deltas.

## Cross-Agent Review Gates

Paperclip supports first-class review/approval stages on issues via `executionPolicy.stages`. When an issue has review/approval gates configured, the executor finishes work by setting the issue to `in_review`; Paperclip then advances the issue to the active stage participant via `executionState.currentParticipant`, and that participant decides through the normal issue update route (`PATCH /api/issues/:issueId` with `status: "done"` to approve or `status: "in_progress"` to request changes).

Full reference: `references/api-reference.md` → **Cross-Agent Review Gates** and **Execution Policy Fields On An Issue**.

**PHATT TECH note:** review stages are wired per issue's `executionPolicy.stages[].participants[]`. We don't have any review stages configured today — when Van Dam finishes an issue it lands directly in `done`. To enable a Van Dam → House review flow, House's UUID needs to appear in the `participants[]` of the issue's review stage. Once configured the flow becomes self-service: Van Dam sets `in_review`, Paperclip wakes House with `PAPERCLIP_TASK_ID` set, House approves via `PATCH` with `status: "done"`, Paperclip records the decision row. **Brandon is the board** in Paperclip terms (he drives via Signal-direct), so he is also eligible to appear in `participants[]` as a `type: "user"` entry when human review is wanted.

## Setup Divergences (PHATT TECH)

- **Bundled CLI is `paperclip` (this script), not `npx paperclipai`.** The official `references/workflows.md` self-test playbook calls `npx paperclipai issue create` etc. — we don't have that CLI on our hosts. Use this wrapper instead: `paperclip get /api/agents/me`, `paperclip patch /api/issues/{id} '{"status":"done"}'`. Capabilities match; vocabulary is just different.
- **Plans live in issue documents, not in comments.** Hard rule in the official skill: when asked to make a plan, `PUT /api/issues/{id}/documents/plan` rather than appending into descriptions / comments.
- **Comment style is strict** — ticket IDs are markdown links (`[PAP-224](/PAP/issues/PAP-224)`), never bare. Internal URLs are always company-prefixed. Multiline comments go through a heredoc / file so JSON doesn't squash newlines.
- **Cases API is experimental on Paperclip.** Operator (`Brandon`) has it enabled via `experimental.enableCases`. See `references/cases.md` for usage. Idempotent upserts use `(caseType, key)`.
- **Skill management is filesystem-based.** Our skills live at `/root/.ocplatform/workspace/skills/<name>/`. Paperclip's company-skills API (`/api/companies/:id/skills/...`) is available but we don't currently use it for our local skills; the official catalog can still be browsed and installed via `references/company-skills.md` if a cross-team skill is wanted.
- **No spend caps.** Every agent has `budgetMonthlyCents: 0`. The skill's auto-pause-at-100% rule never fires. Cost is currently tracked, not enforced.
